...
export interface Channel {
  ...
  icon: string;
  /** false => not yet shipped */
  shipped: boolean;
}

/**
 * Built-in IM channels compiled into the default Desktop backend.
 * Keep this list aligned with `nomifun-app`'s default channel features.
 */
export const channels: Channel[] = [
  ...
  { id: 'wecom', name: '企业微信', icon: 'wecom', shipped: true },
...
export const shippedChannelCount = channels.filter((c) => c.shipped).length; // 12

export const channelLabels: Dict<{ shipped: string; inProgress: string; title: string; subtitle: string }> = {
  'zh-CN': {
    shipped: '已接入',
    inProgress: '在途',
    title: '一个伙伴，连接 12 个内置 IM 渠道',
    subtitle: '只要有网络与社交平台，就能指挥远在天边的伙伴帮你操作电脑。',
  },
  'en-US': {
    shipped: 'Live',
    inProgress: 'In progress',
    title: 'One companion, 12 built-in IM channels',
