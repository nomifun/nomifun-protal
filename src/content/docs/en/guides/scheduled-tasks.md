...
3. **Choose an agent or preset.** Use builtin **Nomi** (no extra install, best for a first run), a detected CLI agent (`claude` / `codex` / `gemini`), or a reusable preset. Newer builds also let you create an unbound scheduled task first and complete its execution setup later. Advanced options can override the workspace and model.
